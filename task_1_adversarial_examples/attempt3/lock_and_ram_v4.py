...
class LockAndRamSolver:
    """
    Lock & Ram Solver: Adaptive epsilon per image.
    ...
    def __init__(
        ...
        self.epsilon_mapping_path = self.log_dir / "lock_and_ram_epsilon_mapping.json"
    ...
    def run(
        ...
        pgd_steps: int = 80,
        num_restarts: int = 5,
        output_filename: str = "submission_ram_final.npz",
        final_iteration: bool = False
    ):
        """Run Lock & Ram strategy with adaptive epsilon - attacks ALL images."""
        print("\n" + "=" * 70)
        if final_iteration:
            print("LOCK & RAM STRATEGY: FINAL Iteration (Restore Iter2 Epsilon for FAILED)")
        else:
            print("LOCK & RAM STRATEGY: Adaptive Epsilon (ALL Images)")
        print("=" * 70)
...
def main():
    parser = argparse.ArgumentParser(
        description='Lock & Ram Strategy: Adaptive Epsilon per Image',
    ...
    parser.add_argument('--log-dir', type=str, default='./logs',
                       help='Log directory')
    parser.add_argument('--output-name', type=str, default='submission_ram_final.npz',
